...
export class SharePointService {
  ...
  public async getListItems(listTitle: string, top: number = 10): Promise<IListItem[]> {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        return data.value.map((item: { Id: number; Title: string; Created: string; Modified: string; Author?: { Title: string; Email: string } }) => ({
          Id: item.Id,
          Title: item.Title,
  ...
  public async createListItem(listTitle: string, itemData: Record<string, string | number>): Promise<boolean> {
    try {
